    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" aria-modal="true" role="dialog">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col">
                {/* Header */}
                <header className="p-4 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-purple-50 to-amber-50 dark:from-purple-900/30 dark:to-amber-900/20">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <input
                            ...
                            className="px-3 py-2 border border-slate-300 dark:border-slate-500 rounded-lg focus:ring-2 focus:ring-purple-500 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 text-sm"
                        />
                    </div>
                </header>
                ...
                <main className="p-6 overflow-y-auto flex-1 space-y-6">
                    {/* Summary */}
                    <div className="bg-gradient-to-r from-purple-50 to-amber-50 dark:from-purple-900/30 dark:to-amber-900/20 p-4 rounded-lg border border-purple-200 dark:border-purple-600">
                        <div className="flex justify-between items-center flex-wrap gap-4">
                            <div>
                                <p className="text-sm text-purple-700 dark:text-purple-400">{t('inventory.suppliersCount')}</p>
                                <p className="text-2xl font-bold text-purple-800 dark:text-purple-300">{Object.keys(purchaseOrderItems).length}</p>
                            </div>
                            <div>
                                <p className="text-sm text-purple-700 dark:text-purple-400">{t('inventory.totalItems')}</p>
                                <p className="text-2xl font-bold text-purple-800 dark:text-purple-300">{totalItems}</p>
                            </div>
                            <div>
                                <p className="text-sm text-purple-700 dark:text-purple-400">{t('inventory.totalAmount')}</p>
                                <p className="text-2xl font-bold text-purple-800 dark:text-purple-300">{formatCurrency(totalAmount)}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {Object.entries(purchaseOrderItems).map(([supplierId, items]) => {
                        ...
                        return (
                            <div key={supplierId} className="bg-white dark:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-600 p-4">
                                {/* Supplier Header */}
                                <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-200 dark:border-slate-600">
                                ...
                                <div className="space-y-3">
                                    {items.map((item, index) => (
                                        <div key={index} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 p-3 bg-gray-50 dark:bg-slate-600 rounded-lg">
                                            <span className="text-slate-500 w-6 hidden sm:inline">{index + 1}</span>
                                            <span className="flex-1 text-slate-800 dark:text-slate-200 font-medium text-sm">{item.itemName}</span>
                                            <div className="flex items-center gap-2">
                                                <input
                                                    type="number"
                                                    min="0"
                                                    ...
                                                    className="w-16 sm:w-20 p-2 border border-slate-300 dark:border-slate-500 rounded-lg text-center focus:ring-2 focus:ring-purple-500 bg-white dark:bg-slate-500 text-slate-800 dark:text-slate-200 text-sm"
                                                />
                                                <span className="text-purple-600 dark:text-purple-400 text-xs sm:text-sm whitespace-nowrap">
                                                    {formatCurrency(item.unitCost)}
                                                </span>
                                                <span className="text-purple-700 dark:text-purple-300 font-bold text-sm sm:text-base whitespace-nowrap">
                                                    {formatCurrency(item.totalCost)}
                                                </span>
                                                <button
                                                    onClick={() => handleRemoveItem(supplierId, index)}
                                                    className="p-1 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded ml-auto sm:ml-0"
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
