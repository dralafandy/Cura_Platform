...
const MobileDrawer: React.FC<MobileDrawerProps> = ({ currentView, setCurrentView, isOpen, onClose, clinicData }) => {
  ...
  const navItems = [
    ...
    { id: 'treatmentDefinitions', label: t('sidebar.treatmentDefinitions'), icon: TreatmentDefinitionsIcon, permission: Permission.SETTINGS_VIEW, adminOnly: false },
  ...
  const groups = [
    {
      ...
      items: filteredNavItems.filter(item => ['dashboard', 'patients', 'doctors', 'treatmentDefinitions', 'suppliers'].includes(item.id)),
    },
    {
