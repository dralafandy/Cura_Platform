...
export class PermissionManager {
  ...
  constructor(userProfile: UserProfile | null, options: PermissionCheckOptions = DEFAULT_PERMISSION_OPTIONS) {
  ...
  hasPermission(permission: Permission): boolean {
    ...
    // Admin bypass
    if (this.options.adminBypass && this.userProfile.role === UserRole.ADMIN) {
      return true;
    }
    ...
    const customPermissions = this.userProfile.custom_permissions || [];
    ...
    if (this.userProfile.permissions && Array.isArray(this.userProfile.permissions)) {
      ...
      return this.userProfile.permissions.some((p: string) => 
   ...
   */
  isDoctor(): boolean {
    return this.userProfile?.role === UserRole.DOCTOR;
  }
   ...
   */
  isAssistant(): boolean {
    return this.userProfile?.role === UserRole.ASSISTANT;
  }
  ...
  isReceptionist(): boolean {
    return this.userProfile?.role === UserRole.RECEPTIONIST;
  }
  ...
  getEffectivePermissions(): Permission[] {
    ...
    if (this.userProfile.role === UserRole.ADMIN) {
    ...
    // Check override mode
    if (this.userProfile.override_permissions) {
      return this.userProfile.custom_permissions || [];
    }
    ...
    if (this.userProfile.permissions && Array.isArray(this.userProfile.permissions)) {
      ...
      this.userProfile.permissions.forEach((p: string) => {
  ...
  getRolePermissions(): Permission[] {
    ...
    return RBAC_ROLE_PERMISSIONS[this.userProfile.role] || [];
  }
 ...
 */
export const createPermissionManager = (userProfile: UserProfile | null): PermissionManager => {
  return new PermissionManager(userProfile);
};
