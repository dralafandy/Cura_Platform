...
const PublicBookingPage: React.FC = () => {
  ...
  const clinicName = params.get('clinic') || t('appName');
  const clinicPhone = params.get('phone') || '';
  const clinicAddress = params.get('address') || '';
  const clinicEmail = params.get('email') || '';
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      const [serviceData, dentistData] = await Promise.all([
  ...
  useEffect(() => {
    ...
    const loadSlots = async () => {
      const response = await getAvailableSlots(form.date, form.dentistId || undefined, { clinicId, branchId });
      if (cancelled) return;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-cyan-50 px-4 py-10 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
        <div className="mx-auto max-w-2xl rounded-[2rem] border border-emerald-200 bg-white p-8 text-center shadow-xl dark:border-emerald-500/20 dark:bg-slate-900">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">{t('publicBooking.requestSubmitted') || 'Request Submitted!'}</h1>
          <p className="mt-3 text-slate-600 dark:text-slate-300">{t('publicBooking.successMessage') || 'Your appointment request has been received and is pending staff approval. We will contact you shortly.'}</p>
          <div className="mt-6 rounded-3xl bg-slate-50 p-5 text-start dark:bg-slate-800">
            <p className="text-sm text-slate-600 dark:text-slate-300"><strong>{t('publicBooking.reservationId') || 'Reservation ID'}:</strong> {reservationId}</p>
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300"><strong>{t('publicBooking.status') || 'Status'}:</strong> {t('publicBooking.pendingApproval') || 'Pending Approval'}</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-cyan-50 px-4 py-8 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      <div className="mx-auto max-w-5xl space-y-6">
        <section className="rounded-[2rem] border border-emerald-200 bg-white p-8 shadow-xl dark:border-emerald-500/20 dark:bg-slate-900">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">{clinicName}</h1>
          <p className="mt-2 text-slate-600 dark:text-slate-300">{t('publicBooking.subtitle') || 'Schedule your visit online'}</p>
        ...
        <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{t('publicBooking.title') || 'Book Your Appointment'}</h2>
            {loading ? (
              <p className="mt-4 text-sm text-slate-500 dark:text-slate-400">{t('common.loading') || 'Loading...'}</p>
            ) : (
              <div className="mt-5 grid gap-4">
                <select value={form.serviceId} onChange={event => handleChange('serviceId', event.target.value)} className="rounded-2xl border border-slate-300 px-4 py-3 dark:border-slate-700 dark:bg-slate-950">
                ...
                <select value={form.dentistId} onChange={event => handleChange('dentistId', event.target.value)} className="rounded-2xl border border-slate-300 px-4 py-3 dark:border-slate-700 dark:bg-slate-950">
                  <option value="">{t('publicBooking.anyDentist') || 'Any Available Dentist'}</option>
                  {dentists.map(item => <option key={item.id} value={item.id}>{item.name} - {item.specialty}</option>)}
                </select>
                <input type="date" value={form.date} onChange={event => handleChange('date', event.target.value)} className="rounded-2xl border border-slate-300 px-4 py-3 dark:border-slate-700 dark:bg-slate-950" />
                <select value={form.time} onChange={event => handleChange('time', event.target.value)} className="rounded-2xl border border-slate-300 px-4 py-3 dark:border-slate-700 dark:bg-slate-950">
                ...
                <button onClick={() => void handleSubmit()} disabled={submitting} className="rounded-2xl bg-emerald-600 px-4 py-3 font-semibold text-white transition hover:bg-emerald-700 disabled:opacity-60">
                  {submitting ? (t('publicBooking.submitting') || 'Submitting...') : (t('publicBooking.submitRequest') || 'Submit Appointment Request')}
                </button>
              </div>
            )}
          </div>
          ...
          <aside className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">{t('publicBooking.bookingSummary') || 'Booking Summary'}</h3>
