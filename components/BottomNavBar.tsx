...
import {
  ...
  LAB_CASE_PERMISSIONS,
  TREATMENT_DEFINITIONS_PERMISSIONS,
  SETTINGS_PERMISSIONS,
  USER_MANAGEMENT_PERMISSIONS,
  FINANCIAL_ACCOUNTS_PERMISSIONS,
  REPORTS_PERMISSIONS,
...
const BottomNavBar: React.FC<BottomNavBarProps> = ({ currentView, setCurrentView }) => {
  ...
  const secondaryNavItems = [
    ...
    { id: 'logout', label: t('auth.logout.button'), icon: LogoutIcon, isLogout: true },
  ];
  ...
  // All users now have access to all navigation items
  const filteredPrimaryItems = primaryNavItems;
  const filteredSecondaryItems = secondaryNavItems;
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-4 rounded-2xl transition-all duration-200 focus:outline-none relative group ${
                        isLogout
                          ...
                          : isActive
                            ? 'bg-gradient-to-r from-purple-600 to-purple-700 text-white shadow-lg shadow-purple-500/25'
                            : 'bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-purple-50 dark:hover:bg-purple-900/20 hover:text-purple-600'
                      }`}
                      aria-label={item.label}
                    >
                      <div className={`mb-2 p-3 rounded-xl transition-all duration-300 ${
                        isLogout
                          ...
                          : isActive 
                            ? 'bg-white/20' 
                            : 'bg-white dark:bg-slate-600 shadow-sm group-hover:shadow-md'
                      }`}>
                        <IconComponent isActive={isActive} />
                      </div>
